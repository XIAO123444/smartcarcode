...
{
    right_down_line=0;          //默认0为没找到
    if(start>MT9V03X_H-4)
    {
        start=MT9V03X_H-4;
    }
    if(end<5){end=5;}
...
int16 Find_Right_Up_Point(int16 start,int16 end)//找四个角点，返回值是角点所在的行数
{
    int i,t;
    int right_up_line=0;
    if(rightlostpoint[0]>=0.9*MT9V03X_H)//大部分都丢线，没有拐点判断的意义
    ...
    if(end<=MT9V03X_H-search_stop)//搜索截止行往上的全都不判
...
int16 Find_Left_Up_Point(int16 start,int16 end)//找四个角点，返回值是角点所在的行数
{
    int i,t;
    int left_up_line=0;
    if(leftlostpoint[0]>=0.9*MT9V03X_H)//大部分都丢线，没有拐点判断的意义
        return left_up_line;
    if(start<end)
    {
        t=start;
...
{            
    ...
    for(i=end;i<=start;i++)
    {
        ...
        if(rightline[i] <rightline[i+10]&&rightline[i] <rightline[i-10]
        )
        {
            result=i;
        return result;
